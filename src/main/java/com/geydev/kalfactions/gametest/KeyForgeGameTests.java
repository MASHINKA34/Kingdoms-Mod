...
public final class KeyForgeGameTests {
    @GameTest(template = "empty", batch = "sculk_key_forge", timeoutTicks = 200)
    public static void assemblyPersistsAndConsumesExactlyOneSet(GameTestHelper helper) {
        ...
        helper.assertValueEqual(forge.progressTicks(), 37, "progress after reload");
        helper.assertTrue(forge.getItem(0).is(ModItems.SCULK_KEY_BOW_FRAGMENT.get()), "bow after reload");
        helper.assertTrue(forge.getItem(1).is(ModItems.SCULK_KEY_SHAFT_FRAGMENT.get()), "shaft after reload");
        helper.assertTrue(forge.getItem(2).is(ModItems.SCULK_KEY_BIT_FRAGMENT.get()), "bit after reload");

        tick(level, pos, forge, KeyForgeBlockEntity.ASSEMBLY_TICKS - 37);
        ...
        helper.assertTrue(forge.getItem(3).is(ModItems.SCULK_KEY.get()), "assembled key");
    ...
    @GameTest(template = "empty", batch = "sculk_key_forge", timeoutTicks = 200)
    public static void slotsRejectForeignPartsResetAndPauseAtOccupiedOutput(GameTestHelper helper) {
        ServerLevel level = helper.getLevel();
        BlockPos pos = helper.absolutePos(new BlockPos(1, 2, 1));
        KeyForgeBlockEntity forge = placeForge(level, pos);
        ...
        helper.assertTrue(forge.canPlaceItem(0, new ItemStack(ModItems.SCULK_KEY_BOW_FRAGMENT.get())), "bow rejected");
        helper.assertTrue(forge.canPlaceItem(1, new ItemStack(ModItems.SCULK_KEY_SHAFT_FRAGMENT.get())), "shaft rejected");
        helper.assertTrue(forge.canPlaceItem(2, new ItemStack(ModItems.SCULK_KEY_BIT_FRAGMENT.get())), "bit rejected");
        helper.assertTrue(!forge.canPlaceItem(3, new ItemStack(ModItems.SCULK_KEY.get())), "result accepted insertion");
        forge.setItem(3, new ItemStack(ModItems.SCULK_KEY.get()));
        ...
        forge.setItem(1, new ItemStack(ModItems.SCULK_KEY_SHAFT_FRAGMENT.get()));
        tick(level, pos, forge, KeyForgeBlockEntity.ASSEMBLY_TICKS);
        helper.assertTrue(forge.getItem(3).is(ModItems.SCULK_KEY.get()), "first result missing");
    ...
    @GameTest(template = "empty", batch = "sculk_key_forge", timeoutTicks = 200)
    public static void shiftClickRoutesPartsAndResultWithoutDuplication(GameTestHelper helper) {
        ServerLevel level = helper.getLevel();
        BlockPos pos = helper.absolutePos(new BlockPos(1, 2, 1));
        KeyForgeBlockEntity forge = placeForge(level, pos);
        Player player = helper.makeMockPlayer(GameType.SURVIVAL);
        player.setPos(pos.getX() + 0.5D, pos.getY() + 0.5D, pos.getZ() + 0.5D);
        player.getInventory().setItem(9, new ItemStack(ModItems.SCULK_KEY_BOW_FRAGMENT.get(), 2));
        player.getInventory().setItem(10, new ItemStack(ModItems.SCULK_KEY_SHAFT_FRAGMENT.get()));
        player.getInventory().setItem(11, new ItemStack(ModItems.SCULK_KEY_BIT_FRAGMENT.get()));
        player.getInventory().setItem(12, new ItemStack(ModItems.GHOST_KEY_BOW_FRAGMENT.get()));

        KeyForgeMenu clientView = new KeyForgeMenu(0, player.getInventory(), pos, KeyForgeType.SCULK);
        ItemStack syncedKey = new ItemStack(ModItems.SCULK_KEY.get());
        helper.assertTrue(!clientView.getSlot(KeyForgeBlockEntity.RESULT_SLOT).mayPlace(syncedKey), "result slot accepts placement");
        clientView.getSlot(KeyForgeBlockEntity.RESULT_SLOT).setByPlayer(syncedKey);
        helper.assertTrue(clientView.getSlot(KeyForgeBlockEntity.RESULT_SLOT).getItem().is(ModItems.SCULK_KEY.get()), "result cannot synchronize");
        clientView.removed(player);
        ...
        helper.assertTrue(forge.getItem(1).is(ModItems.SCULK_KEY_SHAFT_FRAGMENT.get()), "shaft routing");
        helper.assertTrue(forge.getItem(2).is(ModItems.SCULK_KEY_BIT_FRAGMENT.get()), "bit routing");

        tick(level, pos, forge, 30);
        menu.removed(player);
        ...
        helper.assertValueEqual(count(player, ModItems.SCULK_KEY.get()), 1, "key count after shift-click");
        helper.assertTrue(menu.quickMoveStack(player, 3).isEmpty(), "empty result duplicated a key");
        helper.assertValueEqual(count(player, ModItems.SCULK_KEY.get()), 1, "key count after second shift-click");
        menu.removed(player);
        clearForge(level, pos);
        helper.succeed();
    }
    ...
    @GameTest(template = "empty", batch = "sculk_key_forge", timeoutTicks = 200)
    public static void breakingForgeDropsStoredContents(GameTestHelper helper) {
        ServerLevel level = helper.getLevel();
        BlockPos pos = helper.absolutePos(new BlockPos(1, 2, 1));
        KeyForgeBlockEntity forge = placeForge(level, pos);
        fillRecipe(forge);
        ...
        helper.assertValueEqual(countDrops(drops, ModItems.SCULK_KEY_BOW_FRAGMENT.get()), 1, "dropped bows");
        helper.assertValueEqual(countDrops(drops, ModItems.SCULK_KEY_SHAFT_FRAGMENT.get()), 1, "dropped shafts");
        helper.assertValueEqual(countDrops(drops, ModItems.SCULK_KEY_BIT_FRAGMENT.get()), 1, "dropped bits");
        drops.forEach(Entity::discard);
        helper.succeed();
    }

    private static KeyForgeBlockEntity placeForge(ServerLevel level, BlockPos pos) {
        level.setBlock(pos, ModBlocks.SCULK_KEY_FORGE.get().defaultBlockState(), 3);
        return (KeyForgeBlockEntity) level.getBlockEntity(pos);
    }

    private static void fillRecipe(KeyForgeBlockEntity forge) {
        forge.setItem(0, new ItemStack(ModItems.SCULK_KEY_BOW_FRAGMENT.get()));
        forge.setItem(1, new ItemStack(ModItems.SCULK_KEY_SHAFT_FRAGMENT.get()));
        forge.setItem(2, new ItemStack(ModItems.SCULK_KEY_BIT_FRAGMENT.get()));
    }
